...
async def ingest_feeds(context, feeds):
    ...
    # there is an internal race condition in Elasticsearch when the requests wait for
    # acknowledgement from shards, but they never acknowledge requests since the indexes are
    # deleted. At the time of writing, there is no API in Elasticsearch to control any internal
    # timeout on refresh
...
async def ingest_full(parent_context, feed, refresh_timeout):
    ...
    with \
        ...
        await refresh_index(context, activities_schemas_index_name, feed.unique_id,
                            refresh_timeout, 'full')

        await es_ingest_schemas(context, objects_schemas, [objects_schemas_index_name])
        await refresh_index(context, objects_schemas_index_name, feed.unique_id,
                            refresh_timeout, 'full')

        await refresh_index(context, activities_index_name, feed.unique_id,
                            refresh_timeout, 'full')
        await refresh_index(context, objects_index_name, feed.unique_id,
                            refresh_timeout, 'full')
    ...
    with \
        ...
        if href != feed.seed:
            ...
            for index_name in indexes_matching_feeds(indexes_with_alias, [feed.unique_id]):
                await refresh_index(context, index_name, feed.unique_id,
                                    refresh_timeout, 'updates')
