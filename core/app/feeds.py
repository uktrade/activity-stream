...
class MaxemailFeed2(MaxemailFeed):
    ...
    def __init__(
        ...
        super().__init__(unique_id, seed, data_export_url, campaign_url, username, password, page_size)
    ...
    async def pages(cls, context, feed, href, ingest_type):
        ...
        async def gen_sent_activities_and_timestamp():
            ...
            async with feed.session.create_client('s3', **feed.s3_conf) as s3_client:
                ...
                async for record in aiocsv.AsyncDictReader(s3_object['Body'], delimiter=','):
