...
MISSING_CONTENT_TYPE = (
    "Content-Type header was not set. "
...
def authenticator(context, incoming_key_pairs, nonce_expire):
    ...
    async def authenticate(request, handler):
        ...
        (
            ...
            credentials,
        ) = await authenticate_hawk_header(
            context=context,
            ...
            port=str(
                request.url.with_scheme(request.headers["X-Forwarded-Proto"]).port
            ),
        ...
        if not is_authentic:
            request["logger"].warning(
                "Failed authentication (%s)", private_error_message
            )
    ...
    async def _convert_errors_to_json(request, handler):
        try:
            ...
            response = json_response(
                {"details": exception.text}, status=exception.status_code
            )
        except BaseException:
    ...
    async def handle(_):
        ...
        with logged(context.logger.debug, context.logger.warning, "Checking", []):
            ...
            status = (
                ...
                + b"\n"
                + (b"redis:" + (b"GREEN" if is_redis_green else b"RED"))
                + b"\n"
                + (b"elasticsearch:" + (b"GREEN" if is_elasticsearch_green else b"RED"))
                + b"\n"
            )
...
def handle_get_p2_check(parent_context, feeds):
    ...
    async def handle(_):
        ...
        with logged(context.logger.debug, context.logger.warning, "Checking", []):
            ...
            feeds_statuses = await get_feeds_status(
                context, [feed.unique_id for feed in feeds]
            )
            feeds_status_green_if_grace = [
                b"RED"
                if uptime > feed.down_grace_period + grace and status != b"GREEN"
                else b"GREEN"
                for feed, status in zip(feeds, feeds_statuses)
            ]
            all_green = all(
                [status == b"GREEN" for status in feeds_status_green_if_grace]
            )

            status = (
                (b"__UP__" if all_green else b"__DOWN__")
                + (b" (IN_STARTUP_GRACE_PERIOD)" if in_grace_period else b"")
                + b"\n"
                + b"".join(
                    [
                        feed.unique_id.encode("utf-8")
                        + b":"
                        + feeds_status_green_if_grace[i]
                        + b"\n"
...
def handle_get_metrics(context):
    async def handle(_):
        return web.Response(
            body=await redis_get_metrics(context),
            status=200,
            headers={
                "Content-Type": "text/plain; charset=utf-8",
            },
...
def json_response(data, status):
    return web.json_response(data, status=status, headers={"Server": "activity-stream"})
...
def server_logger(logger):
    ...
    async def _server_logger(request, handler):
        ...
        child_logger.info(
            "Receiving request (%s) (%s %s HTTP/%s.%s) (%s) (%s)",
            *(
                (
                    request.remote,
                    request.method,
